<table id="vendor_table" cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
    ...
    <tbody>  
        ...
        {   
            ...
            if($client['ClientMeeting']['vendor_code'])
            {
            ?>              
            <tr class="<?php echo ($key%2)==0?'event':'odd';
            echo ' '.$client['ClientMeeting']['vendor_code'] ?>">
                <td>
                    <?php echo $client['ClientMeeting']['vendor_code']?>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        {
            $.ajax({
                url:'<?php echo $this->Html->url(array("controller"=>"ClientMeetings","action"=>"deleteVendor")) ?>',
                type: 'POST',
                data: {
                    clientmeeting_id : id
                },
