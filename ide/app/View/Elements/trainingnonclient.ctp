<table id="nonclient-table" cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
    <thead>
        ...
        {   
            ...
            foreach ($clienttrainings as $index => $item) :
            if($item['ClientTraining']['nonclient_id'])
            {
            ?>              
            <tr class="<?php echo ($index%2)==0?'event':'odd';
            echo ' '.$item['ClientTraining']['nonclient_id'] ?>">
                <td>
                    <?php echo $item['ClientTraining']['nonclient_id']?>
                ...
                <td>
                    <input type="hidden" value="<?php echo $item['ClientTraining']['clienttraining_id']?>" name="clienttraining_id" >
                    <input type="button" 
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        function deleteNonClient(id)
        {
            $.ajax({
                url:'<?php echo $this->Html->url(array("controller"=>"ClientTrainings","action"=>"deleteNonClient")) ?>',
                type: 'POST',
                data: {
                    clienttraining_id : id
                },
