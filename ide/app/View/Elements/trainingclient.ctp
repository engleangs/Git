<table id="client-table-client" cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
    ...
    <tbody>  
        ...
        {   
            ...
            if($client['ClientTraining']['client_id'])
            {
            ?>              
            <tr class="<?php echo ($key%2)==0?'event':'odd';
            echo ' '.$client['ClientTraining']['client_id'] ?>">
                <td>
                    <?php echo $client['ClientTraining']['client_id']?>
                ...
                <td>
                    <input type="hidden" value="<?php echo $client['ClientTraining']['clienttraining_id']?>" name="clienttraining_id" class="clienttraining_id_client">
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        {
            $.ajax({
                url:'<?php echo $this->Html->url(array("controller"=>"ClientTrainings","action"=>"deleteClient")) ?>',
                type: 'POST',
                data: {
