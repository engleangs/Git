<table id="nonclient-table" cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
    <thead>
        ...
        {   
            ...
            if($item['ClientMeeting']['nonclient_id'])
            {
            ?>              
            <tr class="<?php echo ($index%2)==0?'event':'odd';
            echo ' '.$item['ClientMeeting']['nonclient_id'] ?>">
                <td>
                    <?php echo $item['ClientMeeting']['nonclient_id']?>
                ...
                <td>
                    <input type="hidden" value="<?php echo $item['ClientMeeting']['clientmeeting_id']?>" name="clientmeeting_id" >
                    <input type="button" 
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        {
            $.ajax({
                url:'<?php echo $this->Html->url(array("controller"=>"ClientMeetings","action"=>"deleteNonClient")) ?>',
                type: 'POST',
                data: {
                    clientmeeting_id : id
                },
