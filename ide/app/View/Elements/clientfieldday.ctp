<table id="client-table-client" cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
    ...
    <tbody>  
        ...
        {   
            ...
            foreach ($clientfieldDays as $key=> $client) : 
            if($client['ClientFieldDay']['client_id'])
            {
            ?>              
            <tr class="<?php echo ($key%2)==0?'event':'odd';
            echo ' '.$client['ClientFieldDay']['client_id'] ?>">
                <td>
                    <?php echo $client['ClientFieldDay']['client_id']?>
                    <input type="hidden" name="client[]" value ="<?php
                    echo $client['ClientFieldDay']['client_id'] ?>" />
                </td>
                <td>
                    <?php echo @$client['Client']['client_name_en']?></td>
                <td>
                    <input type="hidden" value="<?php echo $client['ClientFieldDay']['client_field_id']?>" name="client_field_id" class="clienttraining_id_client">
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        {
            $.ajax({
                url:'<?php echo $this->Html->url(array("controller"=>"ClientFieldDays","action"=>"deleteClient")) ?>',
                type: 'POST',
                data: {
                    clienttraining_id : $('.clienttraining_id_client').val()
                },
