...
$this->Html->addCrumb(__('List All'), '/Sroks/listing');
?>
        ...
        <form action="<?php echo $this->Html->url(array()) ?>" id="adminForm" method="post">
    	    <div class="panel panel-primary">
                <div class="panel-heading">
                    <h4><?php echo __('List All Srok') ?></h4>
                    <div class="options">
                        <span>
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
                        <thead>
                            <tr>
                                ...
                                <th>
                                    <?php echo $this->Paginator->sort('khet_code',__('Khet'))?>
                                </th>
                                <th>
                                    <?php echo $this->Paginator->sort('srok_code',__('Code')) ?>
                                </th>
                                <th><?php echo $this->Paginator->sort('srok_name_en',__('Name English'))?>
                                </th>
                                <th><?php echo $this->Paginator->sort('srok_name_kh',__('Name Khmer'))?>
                                </th>
                        ...
                        <tbody>  
                            ...
                            <tr class="<?php echo ($key%2)==0?'event':'odd' ?>">
                                <td><?php echo $value['khet_name_en']?></td>
                                <td colspan="5">
                                    <table>
                                        <?php foreach ($value['sroks'] as $index => $item) :
                                         ?> 
                                        <tr width="100%">
                                            <td><?php echo $item['srok_code']?></td>
                                            <td><?php echo $item['srok_name_en']?></td>
                                            <td><?php echo $item['srok_name_kh']?></td>
                                            <td>
                                            <?php 
                                                echo $this->MyHtml->editButton($item['srok_code']);
                                            ?>
                                            &nbsp;
                                            <?php
                                                echo $this->MyHtml->deleteButton($item['srok_code']);
                                            ?>
                                            </td>
                                            <td>
                                            <?php echo $this->MyHtml->checkBox($item['srok_code'],$key) ?>
                                            </td>
                                        </tr>
                                        <?php endforeach; ?>  
                                    </table>
